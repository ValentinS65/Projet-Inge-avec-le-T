...
noeud* ID_3 (stockage s,int debut, int fin,int profondeur){
   ...
   // printf("Debut : %d , Fin : %d",debut,fin);
    ...
    for (int i=debut; i<fin; i++){
        ...
        if (strcmp(s.tableau[s.ordre_exemple[i]][s.nbr_attributs-1],s.tableau[s.ordre_exemple[i+1]][s.nbr_attributs-1])!=0){
            flag=0;
    ...
    if(profondeur>MAX_DEPTH || fin-debut<nbr_exemples_minimal){
        ...
        EtiquetteCount *etiquette_counts = malloc((fin - debut+1) * sizeof(EtiquetteCount));
        int unique_count = 0;
        ...
        for (int i = debut; i < fin+1; i++) {
            ...
            if (!found) {
                ...
                unique_count++;
                printf("nombre d'etiquette :%d",unique_count);
            }
        }
    ...
    if (indice==-1){
        ...
        actuel->profondeur = profondeur;
        actuel->sous_arbres = NULL;
        //actuel->classe = tirer_au_hasard(actuel->etiquette_counts,actuel->total_etiquettes);
        return actuel;
    }
...
void afficher_arbre(noeud *arbre, int niveau) {
    ...
    //to do je voudrais avoir la valeur de l'attribut dans le neoud.
    if (arbre->nb_sous_arbres == 0) {
        if (arbre->classe!=NULL){
            printf("Feuille: %s\n", arbre->classe);
        }
        else{
            printf("Feuille hasard : %s \n",tirer_au_hasard(arbre->etiquette_counts,arbre->total_etiquettes));
        }
    } else {
        printf("Attribut: %s\n", arbre->attribut);
    }
...
void liberer_arbre(noeud *arbre) {
    ...
    // Libérer les sous-arbres de manière récursive
    for (int i = 0; i < arbre->nb_sous_arbres; i++) {
        liberer_arbre(arbre->sous_arbres[i]);
    }

    // Libérer les allocations dynamiques du nœud actuel
    if (arbre->etiquette_counts != NULL) {
        free(arbre->etiquette_counts);
    }
    if (arbre->indice_sous_arbres != NULL) {
        free(arbre->indice_sous_arbres);
    }
    if (arbre->sous_arbres != NULL) {
        free(arbre->sous_arbres);
    }
